...
function get(object, key) {
    //loop through
    for (let k in object) {
        //assign k as key alter to compare
        let keyProp = k;
        //create return key
        let value = object[k];
        //if match
...
*/
function sum(object) {
    //assign a totalSum variable to be returned after the for loop has executed
    let totalSum = 0;
    //loop through all the key/value pairs in the object
    for (let key in object) {
        ...
        if (Number.isInteger(value)) {
            //add the value to the total sum variable value
            totalSum += value;
        }
    }
    return totalSum;
...
function invert(object) {
    //loop through array
    //return key and value inverted in new array
    const ret = {};
